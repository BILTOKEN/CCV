    ...
    class LauncherForm : Form
    {
        ...
        void ManualCheckUpdate()
        {
            ...
            Task.Run(() =>
            {
                try
                {
                    var verPath = Path.Combine(Program.ROOT, "VERSION");
                    var current = File.Exists(verPath) ? File.ReadAllText(verPath).Trim() : "v4.2.0";
                    using (var wc = new WebClient())
                    {
                        wc.Headers.Add("User-Agent", "CCV");
                        string latest = null;
                        // 先试 Releases API
                        try { var json = wc.DownloadString("https://api.github.com/repos/BILTOKEN/CCV/releases/latest");
                              var ts = json.IndexOf("\"tag_name\":\"") + 12;
                              var te = json.IndexOf("\"", ts);
                              latest = json.Substring(ts, te - ts);
                        } catch {}
                        // 没 Release 就试 Tags API
                        if (string.IsNullOrEmpty(latest)) {
                            try { var json = wc.DownloadString("https://api.github.com/repos/BILTOKEN/CCV/tags");
                                  var ts = json.IndexOf("\"name\":\"") + 8;
                                  var te = json.IndexOf("\"", ts);
                                  latest = json.Substring(ts, te - ts);
                            } catch {}
                        }
                        if (string.IsNullOrEmpty(latest))
                            this.Invoke((Action)(() => ShowToast("暂无可用更新")));
                        else if (latest == current)
                            this.Invoke((Action)(() => ShowToast("已是最新版本 " + current)));
                        else
                            CheckForUpdate();
                    }
                }
                catch { this.Invoke((Action)(() => ShowToast("检查失败，请检查网络"))); }
            });
        }
        ...
        void CheckForUpdate()
        {
            Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        wc.Headers.Add("User-Agent", "CCV");
                        var json = wc.DownloadString("https://api.github.com/repos/BILTOKEN/CCV/releases/latest");
                        var tagStart = json.IndexOf("\"tag_name\":\"") + 12;
                        var tagEnd = json.IndexOf("\"", tagStart);
                        var latest = json.Substring(tagStart, tagEnd - tagStart);

                        if (latest == current) return;

                        // 从 Release 资产中找 ZIP 下载链接
                        var dlUrl = "";
                        var dlStart = json.IndexOf("\"browser_download_url\":\"");
                        if (dlStart > 0)
                        {
                            dlStart += 24;
                            var dlEnd = json.IndexOf("\"", dlStart);
                            dlUrl = json.Substring(dlStart, dlEnd - dlStart);
                        }

                        this.Invoke((Action)(() =>
                        {
                            var r = MessageBox.Show(
                                "发现新版本 " + latest + "（当前 " + current + "）\n\n是否自动更新？\n更新不会影响 workspace 里的文件。",
                                "CCV 更新", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                            if (r != DialogResult.Yes) return;

                            var updForm = new Form
                            {
                                Text = "正在更新 CCV...",
                            ...
                            var label = new Label { Text = "正在下载 " + latest + "...", ForeColor = Color.White, Location = new Point(20, 20), Size = new Size(360, 24), Font = new Font("Segoe UI", 10) };
                            var bar = new ProgressBar { Location = new Point(20, 55), Size = new Size(350, 28), Style = ProgressBarStyle.Marquee };
                            var detail = new Label { Text = "", ForeColor = Color.FromArgb(160, 160, 180), Location = new Point(20, 95), Size = new Size(350, 20), Font = new Font("Segoe UI", 8) };
                            updForm.Controls.AddRange(new Control[] { label, bar, detail });
                            updForm.Shown += async (s2, e2) =>
                            {
                                try
                                {
                                    var tmpZip = Path.Combine(Path.GetTempPath(), "CCV_update_" + latest + ".zip");
                                    detail.Text = "下载中...";
                                    if (string.IsNullOrEmpty(dlUrl))
                                        wc.DownloadFile("https://github.com/BILTOKEN/CCV/releases/download/" + latest + "/CCV.zip", tmpZip);
                                    else
                                    ...
                                    label.Text = "正在安装更新...";
                                    // 解压到临时目录
                                    var tmpDir = Path.Combine(Path.GetTempPath(), "CCV_extract");
                                    if (Directory.Exists(tmpDir)) Directory.Delete(tmpDir, true);
                                    System.IO.Compression.ZipFile.ExtractToDirectory(tmpZip, tmpDir);

                                    // 复制 _runtime（覆盖引擎）
                                    var srcRuntime = Path.Combine(tmpDir, "_runtime");
                                    if (Directory.Exists(srcRuntime))
                                        CopyDir(srcRuntime, Path.Combine(Program.ROOT, "_runtime"));

                                    // 替换 EXE — 写批处理延迟替换
                                    var parentDir = Directory.GetParent(Program.ROOT).FullName;
                                    if (parentDir == null) parentDir = Path.Combine(Program.ROOT, "..");
                                    var srcExe = Path.Combine(tmpDir, "CCV v4.2 启动器.exe");
                                    var dstExe = Path.Combine(parentDir, "CCV v4.2 启动器.exe");
                                    if (File.Exists(srcExe))
                                    {
                                        var bat = Path.Combine(Path.GetTempPath(), "ccv_update.bat");
                                        File.WriteAllText(bat,
                                            "@echo off\r\n" +
                                            "timeout /t 2 /nobreak >nul\r\n" +
                                            "copy /Y \"" + srcExe + "\" \"" + dstExe + "\"\r\n" +
                                            "start \"\" \"" + dstExe + "\"\r\n" +
                                            "del \"%~f0\"\r\n");
                                        Process.Start(new ProcessStartInfo { FileName = bat, WindowStyle = ProcessWindowStyle.Hidden });
                                    }
        ...
        void BuildUI()
        {
            ...
            aboutLink.MouseLeave += (s, e) => aboutLink.ForeColor = Color.FromArgb(80, 200, 120);

            var updateLink = new Label
            {
                Text = "检查更新",
                Font = new Font("Segoe UI", 9, FontStyle.Underline),
            ...
            updateLink.Click += (s, e) => ManualCheckUpdate();
            updateLink.MouseEnter += (s, e) => updateLink.ForeColor = Color.Cyan;
            updateLink.MouseLeave += (s, e) => updateLink.ForeColor = Color.FromArgb(100, 180, 255);

            statusBar.Controls.AddRange(new Control[] {
                apiDot, apiStatusText, dashboardDot, dashboardStatusText,
                proxyDot, proxyStatusText,
                statusLabel, devLabel, aboutLink, updateLink
            });
